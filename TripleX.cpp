// Program to prompt a user to guess the product and sum of three numbers
...
bool PlayGame(int LevelDifficulty, int GuessesLeft) {
    ...
    if (GuessSum == CodeSum && GuessProduct == CodeProduct && GuessesLeft >= 1) {
...
int main() {
    ...
    int GuessesLeft = 9;        // Integer to hold the total amount of guesses
    int const MaxLevel = 5;     // Constant integer to hold the max game level

    PrintIntroduction();

    while (LevelDifficulty <= MaxLevel && GuessesLeft >= 1) {    
        bool bLevelComplete = PlayGame(LevelDifficulty, GuessesLeft);
